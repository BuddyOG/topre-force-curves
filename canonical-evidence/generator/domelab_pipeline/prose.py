...
Nothing here interprets a metric perceptually. Each replacement states what the
quantity mechanically is and, where the original asserted a perceptual meaning,
records that no such correspondence has been validated.
"""
import re
...
VIEWER_CLAIMS = [
    ...
    ("The distance travelled when the switch bottoms out at the end of the stroke.",
     "The distance to detected force-wall onset, an operational bottom-out proxy rather than observed contact.", 1),
    ("bottom-out wall", "force-wall pattern", 3),
    ("Full-stroke press work", "Press work to force-wall", 3),
    ("peak-relative sharpness per mm", "peak-normalized force-loss secant per millimetre", 1),
    ("the dome-switch industry's tactile-ratio standard, where linear = 0% and higher is crisper",
     "a relative peak-to-valley force-drop ratio (linear = 0%); no correspondence with perceived "
     "tactility has been validated", 1),
    ("the total work your finger does across the keystroke",
     "the one-way mechanical work integral to detected force-wall onset", 2),
    ("the closest energy proxy for how hard the dome feels to overcome",
     "the one-way mechanical work integral to collapse", 2),
    ("The industry standard for measuring how tactile the switch feels.",
     ...
     "descriptor; not a validated perceptual score.", 1),
    ("These quantities describe different aspects of the stroke and are not a single universal "
     "measure of feel.",
     "SNAP, DROP, DROP TRAVEL, DROP RATE, NDR, STEEPEST DROP, RAMP and the work integrals are "
     "mechanical descriptors of the measured curve. None has yet been validated as a universal "
     "perceptual score.", 1),
...
WORK_LANGUAGE = {
    "full_stroke": "press work to force-wall \u2014 the one-way mechanical work integral to detected "
                   "force-wall onset",
